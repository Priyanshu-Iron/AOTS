...
def process_frame(frame):
    ...
    if tracking_enabled and current_obj_id:
        try:
            ...
            tracking_enabled = False
            current_obj_id = None
            cv2.putText(frame, "Tracking Error", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    else:
...
def generate_frames():
    while True:
        ...
        if camera.cap is None or not camera.cap.isOpened():
            ...
            cv2.putText(frame, "Camera stopped", (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        else:
            frame = camera.get_frame()
            if frame is None or not isinstance(frame, np.ndarray) or frame.size == 0:
                logger.warning("Failed to get valid frame from camera")
                frame = np.zeros((240, 320, 3), dtype=np.uint8)
                cv2.putText(frame, "No camera feed", (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            else:
                frame = process_frame(frame)
                if frame is None:
                    frame = np.zeros((240, 320, 3), dtype=np.uint8)
                    cv2.putText(frame, "Processing error", (20, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        fps = 1 / (time.time() - start_time)
        cv2.putText(frame, f"FPS: {fps:.2f}", (20, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
